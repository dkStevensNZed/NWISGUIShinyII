if (interactive()) {
  ...
  source('CreateandPlotAverageShiny.r')
...
#               ),
        fluidRow(
          column(3,
            h5('Site'),
            checkboxGroupInput(inputId="site",label=NA,
              choices=names(r.x[['WY2016']][[1]]$Data),
              selected='RICH')
                 ),
          column(3,
            h5('Parameter'),
            radioButtons(inputId="parameter",label=NA,
              choices=names(pCBG),
              selected='Tempw')
          )
        ),
        fluidRow(
          column(7,
            h5('Analysis'),
            radioButtons(inputId="analysis",label=NA,
              choices=c('No filter','Godin',
                        'Butterworth','Harmonics','Loess','Moving average',
                        'Period average','Trend break analysis',
                        'Gap analysis','...'),
              selected='No filter')
          ),
          column(5,
  ...
  server <- function(input, output) {
    ...
    output$tsPlot <- renderPlot({
      ...
      ds.cur <- getData(Result.all.lst=r.x,wy=WY,st=ST,vars=VR)
#      ds.cur <- extractData(Result.all.lst=r.x,wy=WY,st=ST,vars=VR)
      ...
      assign(cols,add.alpha(getCols(length(ST)),0.5), envir = .GlobalEnv)
      ...
      box()
      plotAverages(ds.cur,input,pType='Time series',type=input$analysis)
...
#      ds.cur <- extractData(Result.all.lst=r.x,wy=WY,st=ST,vars=VR)
      nCol <- min(nST,3)
      nRow <- ceiling(nST/3)
      par(mfrow=c(nRow,nCol),cex.main=0.8)
