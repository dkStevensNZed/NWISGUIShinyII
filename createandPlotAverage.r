...
createandPlotAverage <- function(avgtype='Period average',p.options=NULL,mingap=15,maxgap=60,cols){
...
#  browser()
  ...
  d.cur <- ds.cur[which(ds.cur$dateTime >= svalue(geStart) & ds.cur$dateTime <= svalue(geEnd)),]
  cat('done\n')
  sf <- 1
  limits <- as.numeric(c(svalue(geyMin),svalue(geyMax)))
...
#    
    sapply(AvgPers,function(wa) {  # send the averaging period or other parameter to sort out the average
      ...
      sapply(st.u,function(st) {   # separate by site
        ...
        if(svalue(grbPlot) == 'Time series') {   # may want to bundle all of hhe parameters into one unit
...
#            pMean <- by(d.avg$ResultU,IND=list(d.avg$WaterYear,d.avg$site_no),FUN=mean, na.rm=T)
        ...
        } else if(svalue(grbPlot) == 'Cumul. dist.') {
    ...
    if(svalue(gchkLegendPos)) {    
      legPos <- svalue(gcmbLegendPos); if(length(unique(d.cur$site_no)) > 1) legPos <- 'topright'
      leg <- as.character(AP)
      col.leg <- col.leg[-1]
...
#cat('pm 0b: ','\n'); print(pMean); browser()
      ...
      sapply(1:(length(w.per)-1),function(ip) {
        sInd <- which(between(d.cur[,'dateTime'],w.per[ip],w.per[ip+1],flgIncl=F) &
                    d.cur$site_no == st)
